...
/** The shipped defaults, kept as a named constant so the settings drawer's
 * "Reset to defaults" restores exactly what a first run would have used.
 * Zero Trust credentials and routing rules are user data, not tuning, so
 * reset deliberately leaves them alone. */
...
interface ConnectionState {
  ...
  /** Restores the shipped tuning defaults, leaving Zero Trust credentials
   * and routing rules untouched. */
  resetProfile: () => void
...
export const useConnectionStore = create<ConnectionState>((set, get) => ({
  ...
  connect: async () => {
    ...
    try {
      ...
      // from a normal connection failure, so it becomes the dial's "setup"
      // view with its own explanation, not a generic connection error.
  ...
  resetProfile: () =>
    set((s) => ({ profile: { ...s.profile, ...DEFAULT_TUNING } })),
