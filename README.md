...
- **Backend**: Rust, using [`portable-pty`](https://docs.rs/portable-pty) to spawn the real `aether` binary (v1.3.0) in a genuine pseudo-terminal. When TUN mode is enabled, a second process ([sing-box](https://github.com/SagerNet/sing-box) v1.13.14) is spawned to create a system-wide TUN interface that routes all traffic through Aether's SOCKS5 proxy. Your chosen profile — protocol, scan mode, IP version, MASQUE transport (HTTP/3 or HTTP/2), obfuscation profile, quick reconnect — is passed as CLI flags/environment up front, so Aether's interactive prompts normally never appear; a background thread still watches the output and can answer any prompt that does, while forwarding every line live to the GUI's log panel.
